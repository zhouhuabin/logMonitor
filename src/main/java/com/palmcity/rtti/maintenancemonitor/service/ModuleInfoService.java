 ...
 *          </table>
 */
public class ModuleInfoService extends BaseAction{
	...
	public void commonActionParser() {
		try {
			...
			{
				Map<String, Object> resultMap=DaoTool.getModuleInfoDao().insertDetail(paramMap);
				String array = JsonUtil.getJsonStringFromMap(resultMap);
				LogFileScanSchedule.restartScanThread(Integer.parseInt(paramMap.get("module_type_id").toString()));
			...
			if(method.equals("list"))
			{
				...
				List<ModuleInfo> resultList=DaoTool.getModuleInfoDao().queryModuleInfoList(null);
			...
			{
				ArrayList<ModuleInfo> list=LogFileScanSchedule.moduleInfoMapByTypeId.get(Integer.parseInt(paramMap.get("module_type_id").toString()));
				
				for(int i=0;i<list.size();i++)
				{
					if(list.get(i).getModule_ID()==Integer.parseInt(paramMap.get("module_id").toString()))
					{
						String array = JsonUtil.getJsonStringFromObject(list.get(i));
						sendJsonText(array.toString());
					}
				}
			}
			...
			{
				Map<String, Object> resultMap=DaoTool.getModuleInfoDao().updateModuleInfoById(paramMap);
				String array = JsonUtil.getJsonStringFromMap(resultMap);
				LogFileScanSchedule.restartScanThread(Integer.parseInt(paramMap.get("module_type_id").toString()));
				sendJsonText(array.toString());
			}
			...
			{
				Map<String, Object> resultMap=DaoTool.getModuleInfoDao().deleteModuleInfoById(paramMap);
				String array = JsonUtil.getJsonStringFromMap(resultMap);
				LogFileScanSchedule.restartScanThread(Integer.parseInt(paramMap.get("module_type_id").toString()));
				sendJsonText(array.toString());
			}
			
			if(method.equals("moduleinfocombobox"))
			{
				HashMap<String, Object> resultMap=new HashMap<String, Object>();
				List<ModuleInfo> list=DaoTool.getModuleInfoDao().queryModuleInfoList(paramMap);
				...
				for(int i=0;i<list.size();i++)
				{
					...
					infoMap.put("id", list.get(i).getModule_ID()+"");
					infoMap.put("text",list.get(i).getModule_Desc());
